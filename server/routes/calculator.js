...
router.route('/equal').post(async(req,res)=>{
    ...
    try{
        if(!equation){
            ...
            let insert = await pool.calculator.query("insert into calculator(result,equation) values($1,$2)",[result,equation])
            let getCount = await pool.calculator.query("select count(id) from calculator")
            let getData = await pool.calculator.query("select result,equation from calculator")
...
router.route('/equal').get(async(req,res)=>{
    try{
        const selectAll = await pool.calculator.query("select * from calculator order by id desc")
...
router.route('/history/delete').get(async(req,res)=>{
    await pool.calculator.query("truncate calculator;alter sequence calculator_history_id_seq restart with 1")
